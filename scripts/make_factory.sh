...
if [[ ! -d "$SOURCE_DIR" ]]; then
  echo "ERROR: WLED directory not found: $SOURCE_DIR"
  exit 2
fi
...
cd "$SOURCE_DIR"
for env in "$@"; do
  build_dir="$SOURCE_DIR/.pio/build/$env"
  ...
  chip_disp="$chip"
  # Older esptool.py can mis-identify ESP32 bootloaders as ESP8266 unless a
  # chip is supplied. This pipeline only targets ESP32-family boards.
  ...
  factory_tmp="$SOURCE_DIR/build_output/factory/WLED_${normal_git_ref}_${target_slug}.bin"
  factory_dest="$REPO_ROOT/$OUT_DIR/WLED_${normal_git_ref}_${target_slug}.bin"

  echo "Factory merge: $env (chip='$chip_disp', boot_off=$boot_off) -> $factory_dest"
  mkdir -p "$SOURCE_DIR/build_output/factory"
