...
WLED_URL     ?= https://github.com/wled/WLED.git
...
export IOT_SSID WPA_KEY GITHUB_WORKSPACE FLASH_MODE FLASH_SIZE GIT_REF OUT_DIR WLED_DIR PIO_NO_ANSI PIO_DISABLE_PROGRESSBAR

# Wrapper that runs PlatformIO inside your builder container with host-mounted ~/.platformio
WITH_BUILDER ?= cd "$(REPO_ROOT)" && WLED_DIR="$(WLED_DIR)" "$(REPO_ROOT)/scripts/with_builder.sh"
...
clone: ## Shallow clone WLED at a particular Git ref (tag/branch/SHA). Usage: make clone GIT_REF=v16.0.0 [GH_PAT=token]
	@if [[ -z "$(GIT_REF)" ]]; then
		echo "ERROR: missing GIT_REF. Example: make clone GIT_REF=v16.0.0"
		exit 2
	fi

	if [[ -d "$(WLED_DIR)/.git" ]]; then
		echo "WLED already present at $(WLED_DIR)"
		exit 0
	fi
...
checkout: ## Checkout a tag/branch/commit of existing WLED. Usage: make checkout GIT_REF=v16.0.0 [GH_PAT=token]
	@if [[ -z "$(GIT_REF)" ]]; then
		echo "ERROR: missing GIT_REF (tag, branch, or SHA). Example: make checkout GIT_REF=v16.0.0"
		exit 2
	fi
	...
	# Ensure repo exists
	if [[ ! -d "$(WLED_DIR)/.git" ]]; then
		echo "WLED not found; cloning shallow..."
...
build-prep:
	@install -m 0644 "$(PLATFORM_OVERRIDE_INI)" "$(REPO_ROOT)/$(WLED_DIR)/platformio_override.ini"
	"$(REPO_ROOT)/scripts/apply_wled_patches.sh"
	...
	if [[ -n "$$selected_envs" ]]; then
		...
		echo "Preparing PlatformIO default_envs from WLED/platformio.ini"
	fi
...
# Produces WLED_x.y.z_CamelcasedTarget_OTA.bin from the WLED build output.
move-bins: ## Create OTA bins (renamed firmware.bin) for each env in PIO_ENVS
	...
	for env in $(PIO_ENVS); do
		...
		if compgen -G "$$release_glob" > /dev/null; then
			...
			for rel in build_output/release/*.bin; do
				...
				if [[ "$$(stat -c%s "$$rel")" == "$$fw_size" ]]; then
					...
					tmp="$${bin_name%.bin}"
					# remove leading WLED_<ver>_ if present
					tmp="$${tmp#*_*_}"
		...
		if [[ ! -f "$$ota_src" ]]; then
			echo "ERROR: missing $$ota_src (did the build succeed for env $$env?)"
			exit 2
		fi
		...
		ota_dest="../$(OUT_DIR)/WLED_$${normal_git_ref}_$${target_name// /_}_OTA.bin"

		echo "OTA: $$ota_src -> $$ota_dest"
		cp -f "$$ota_src" "$$ota_dest"
	done


# Produces WLED_x.y.z_CamelcasedTarget.bin by merging bootloader+partitions+boot_app0+firmware into one image.
...
ship-local:
	@tag=$$(curl -fsSL "https://api.github.com/repos/wled/WLED/releases?per_page=50" | jq -r ' \
